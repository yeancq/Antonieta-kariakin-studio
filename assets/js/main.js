(function () {
  ...
  /* Scroll reveal: fade/slide sections and cards in as they enter the
     viewport, and reset them so the effect replays when scrolling back
     up. Staggers grid children (services, packs, cards) by 90ms. */
  var revealEls = Array.prototype.slice.call(document.querySelectorAll(".reveal"));

  if (revealEls.length) {
    var staggerParents = [".service-grid", ".pack-grid", ".booking-grid", ".flow-list", ".contact-grid"];
    var staggerSelector = staggerParents.join(",");

    revealEls.forEach(function (el) {
      var parent = el.closest(staggerSelector);
      if (!parent) { return; }
    ...
    if ("IntersectionObserver" in window) {
      ...
      /* No IntersectionObserver support: show everything, no animation. */
